...
class FlowField {
  ...
  lookup(l) {
    const x = Math.floor(constrain(l.x / this.resolution, 0, this.cols - 1));
    const y = Math.floor(constrain(l.y / this.resolution, 0, this.rows - 1));
...
export const perlinField = (width, height, resolution, xinc = 0.009, yinc = 0.009, styles = {}) => {
  const ff = new FlowField(width, height, resolution, styles);
